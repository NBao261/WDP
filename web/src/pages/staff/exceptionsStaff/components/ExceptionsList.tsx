...
export default function ExceptionsList({
  ...
  const logic = useExceptionsListLogic({
    exceptionsList,
    searchQuery,
    filterStatus,
    onSearchChange,
  ...
  const currentData = logic.filteredAndSortedList.slice(
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col flex-1 min-h-0 overflow-hidden relative">
      ...
      <ExceptionsPagination
        ...
        filteredAndSortedList={logic.filteredAndSortedList}
        itemsPerPage={logic.itemsPerPage}
      />
    </div>
  );
